...
def get_parser():
    ...
    parser.add_argument('--model', dest='model_name',
                        help='model name',
                        default='SACR', type=str)
    args = parser.parse_args()
    return args

def SACR(images, labels, cfg, window_size, step_size, batch_size=128):
    ...
    loop = tqdm(zip(images, labels), total=len(images), desc="Generating Pseudo Labels")
    idx = 0
    print(len(images))
    for image, label in loop:
        ...
        for y in range(0, height - window_size[1], step_size):
            for x in range(0, width - window_size[0], step_size):
                ...
                crops.append(new_clip_preprocess(cropped_img))
        ...
        out_json = f"text/{cfg.DATASET.NAME}_pseudo_label_{args.model_name}.json"
        with open(out_json, mode='a') as cls_json:
...
if __name__ == '__main__':
    ...
    if args.model_name == 'SACR':
        SACR(train_images, train_labels, cfg, window_size=(w, w), step_size=s)
